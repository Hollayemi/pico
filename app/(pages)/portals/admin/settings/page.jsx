...
import {
  ...
  Sun, Moon, Monitor, Upload, Loader2, ChevronDown, ChevronUp,
  GraduationCap, Home, Bus
} from "lucide-react";
import {
  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden">
      ...
      <div className="p-4 space-y-2">
        ...
        <button
          onClick={onAddItem}
          className="flex items-center gap-1.5 mt-2 text-xs text-brand-600 hover:text-brand-800 font-medium"
        >
...
const FeeStructureTab = () => {
  ...
  // Local state: deep copy of fee structure for editing
  ...
  // ── Grand total per term across all categories (summary) ─────
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Class → Category Mapping</p>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          {Object.entries(CATEGORY_META).map(([key, meta]) => (
            <div key={key} className={`flex items-start gap-2 p-2 rounded-lg border text-xs ${meta.color}`}>
              <span className="text-base leading-none">{meta.icon}</span>
              <div>
                <p className="font-semibold">{meta.label}</p>
                <p className="opacity-70 mt-0.5">{meta.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {Object.keys(feeState).length > 0 && (
        <div className="border border-gray-200 rounded-xl overflow-hidden">
          ...
          <div className="px-4 py-2 bg-gray-50 border-t border-gray-100">
            <p className="text-xs text-gray-400">Click a row to edit that category's fee items below.</p>
          </div>
        </div>
      )}
      ...
      {activeCategory && (
        <div className="border border-gray-200 rounded-2xl overflow-hidden">
          {/* Header */}
          <div className={`px-5 py-4 border-b border-gray-200 flex items-center justify-between ${CATEGORY_META[activeCategory]?.color?.replace('text-', 'text-').split(' ').filter(c => c.startsWith('bg-')).join(' ')} bg-opacity-30`}>
            <div>
              <div className="flex items-center gap-2">
          ...
          <div className="p-5 space-y-4">
            ...
            {/* Per-category term totals comparison */}
