...
import {
  ...
  FileText, ChevronLeft, ChevronRight, Eye, User,
  Shield, MessageSquare, Check, SlidersHorizontal,
  Download, BookOpen, Paperclip, PenLine, Users
} from "lucide-react";
...
const MOCK_APPLICANTS = Array.from({ length: 30 }, (_, i) => {
  ...
  const screeningStatuses = ["Pending", "Verified", "Rejected", "Pending", "Pending", "Verified"];

  const docs = {};
  DOC_LIST.forEach((d, di) => { docs[d.key] = di < (i % 3 === 0 ? 3 : DOC_LIST.length); });
  ...
  return {
    ...
    screeningStatus: screeningStatuses[i % screeningStatuses.length],
    assignedOfficer: i % 3 === 0 ? SCREENING_OFFICERS[i % SCREENING_OFFICERS.length] : null,
    notes: i % 4 === 0 ? "Documents are complete. Academic record looks good." : "",
    docs,
    dateApplied: new Date(Date.now() - i * 86400000).toISOString().split("T")[0],
  };
});
...
const ScreeningModal = ({ applicant, onClose, onSave }) => {
  if (!applicant) return null;

  const [docs, setDocs] = useState({ ...applicant.docs });
  const [status, setStatus] = useState(applicant.screeningStatus);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden">
              ...
              {DOC_LIST.map(doc => (
                <label key={doc.key}
                  ...
                  <div className="flex-1">
                    <p className={`text-sm font-medium ${docs[doc.key] ? "text-green-800" : "text-gray-700"}`}>
                      {doc.label}
                    </p>
                    {doc.required && (
        ...
        <div className="border-t border-gray-100 px-6 py-4 flex items-center justify-end gap-3 flex-shrink-0 bg-gray-50">
          ...
          <button
            onClick={() => { onSave({ ...applicant, docs, screeningStatus: status, assignedOfficer: officer, notes }); onClose(); }}
            className="flex items-center gap-2 px-5 py-2 bg-brand-600 text-white text-sm rounded-xl hover:bg-brand-700 font-semibold"
          >
...
export default function ScreeningPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return applicants.filter(a =>
      (!search || `${a.surname} ${a.firstName}`.toLowerCase().includes(q) || a.id.toLowerCase().includes(q)) &&
      (!statusFilter || a.screeningStatus === statusFilter)
    );
  ...
  const stats = useMemo(() => ({
    ...
    pending: applicants.filter(a => a.screeningStatus === "Pending").length,
    verified: applicants.filter(a => a.screeningStatus === "Verified").length,
    rejected: applicants.filter(a => a.screeningStatus === "Rejected").length,
    assigned: applicants.filter(a => a.assignedOfficer).length,
  }), [applicants]);
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {paginated.map(applicant => {
          const docsComplete = DOC_LIST.filter(d => d.required).every(d => applicant.docs[d.key]);
      ...
      {selected && (
        <ScreeningModal applicant={selected} onClose={() => setSelected(null)} onSave={handleSave} />
      )}
    </div>
  );
}
